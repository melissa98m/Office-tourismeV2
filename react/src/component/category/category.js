...
function Category() {
    ...
    const handleDataChange = async (dataChange, message) => {
        ...
        if (message && message === 'edit'){
            setToastMessage({message: "Categorie modifiée !", severity: "success"});
            setShowToast(true);
        } else if(message && message === 'delete') {
            setToastMessage({message: "Categorie supprimée !", severity: "success"});
            setShowToast(true);
        }
    }
    ...
    return <Container maxWidth="md" id="Category">
        <Paper sx={{display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', py: 10}}>
            <Typography variant="h3" sx={{textAlign: "center"}} gutterBottom>Categorie d'articles</Typography>
            {loading ? (
