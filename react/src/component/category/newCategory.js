...
function NewCategory(props) {
    ...
    let newCategoryForm = async () => {
        try {
            ...
            if (res.status === 200) {
                ...
                setName("");
                setToastMessage({message: "Categorie ajouté ! Vous pouvez en ajouter une autre", severity: "success"});
                setShowToast(true);
            } else {
    ...
    return (<Box>
        ...
        >
            <Box className="modal-crud modal-crud-Category" sx={{bgcolor: 'background.default'}}>
                <Typography variant="h4" sx={{textAlign: 'center', mb: 4}} id="new-Category-title">Nouvelle Categorie d'article</Typography>
