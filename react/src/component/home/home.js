...
function Home() {
    ...
    const handleDataChange = async (dataChange) => {
     ...
     return <Container maxWidth="xl" id="home">
         <Typography variant="h3" sx={{textAlign: "left"}} gutterBottom>Bienvenue sur le site office du tourisme</Typography>

        <Typography variant="h5" sx={{textAlign: "center"}} gutterBottom>Les derniers lieux</Typography>
         <Typography variant="h5" sx={{textAlign: "center"}} gutterBottom>Les derniers articles</Typography>
         {loading ? (
             <Typography variant="h5" sx={{textAlign: "center"}} gutterBottom>Chargement des article...</Typography>
         ) : (
             <Box sx={{ maxWidth: '90%' }}>
                 {data.slice(0,5).map(({id, name_article, content_article, image, category, place , created_at}) => {
                     return (
         ...
         <Typography variant="h5" sx={{textAlign: "center"}} gutterBottom>Les derniers évenements</Typography>
