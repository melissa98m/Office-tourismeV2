                ...
                <Box sx={{ maxWidth: '100%' }}>
                    ...
                    <TableContainer sx={{ mt:4 }}>
                        <Table size="small">
                            ...
                            <TableBody>
                                {data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(({id, name_type}) => {
                                    return (
                                        <TableRow hover role="checkbox" tabIndex={-1} key={id}>
                                            <TableCell>{id}</TableCell>
                                            <TableCell sx={{fontWeight: 'bold'}}>{name_type}</TableCell>
                                            <TableCell>
                                                <Box sx={{display: 'flex', justifyContent: 'right'}}>
                                                    <EditType updateValue={{id, name_type, data}} handleDataChange={handleDataChange} />
                                                    <DeleteType deleteValue={{id, name_type, data}} handleDataChange={handleDataChange}/>
                                                </Box>
                                            </TableCell>
                                        </TableRow>
                                    )
